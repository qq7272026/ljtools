...
# 👈 核心新增：引入了 QTextBrowser 和 QHBoxLayout 用于绘制 AI 聊天框
...
# ================= 🚨 请在这里填入你的智谱 API Key =================
ZHIPU_API_KEY = "bf9d4e6bca184341aca59347f3042229.VgOX41l7VVfB0p0o"  # 例如: "1234567890abcdef.12345678"
...
class DeepSeekAnalyzerDialog(QDialog):
    def __init__(self, sql_text, plan_text, parent=None):
        ...
        # ⭐ 核心修复 1：使用全局默认的 Profile！
        # 这样它的生命周期就和你的整个 PyQt 主程序一致，避免了弹窗销毁导致的 LocalStorage 丢失
        self.profile = QWebEngineProfile.defaultProfile()

        # 伪装为标准的 Windows Chrome 浏览器
        ...
        # ⭐ 核心修复 2：使用绝对路径，并同时设置 CachePath 和 StoragePath
        ...
        # 确保开启了 LocalStorage 权限（DeepSeek 存放 Token 的关键所在）
    ...
    def on_load_finished(self, success):
        ...
        # ⭐ 核心改进：引入状态机机制的自动化脚本
        # 严格按照顺序执行：开启新对话 -> 点击专家模式 -> 填入文本 -> 发送
        js_code = f"""
        (function() {{
            let attempts = 0;
            let step = 0; // 0: 等待/点击新对话, 1: 等待/点击专家模式, 2: 填入并发送

            let fillInterval = setInterval(function() {{
                attempts++;

                // 第 0 步：点击页面上的“开启新对话”按钮
                if (step === 0) {{
                    let newChatBtn = document.querySelector('div[role="button"][aria-label*="新对话"]') || 
                                     document.querySelector('.ds-icon-button') ||
                                     Array.from(document.querySelectorAll('div, button, span')).find(el => el.textContent && el.textContent.trim() === '开启新对话');

                    if (newChatBtn && attempts <= 3) {{
                        newChatBtn.click();
                        step = 1; // 点击成功，进入下一步
                        return;   // 等待下一次定时器循环再执行下一步，给页面动画留时间
                    }}
                    // 如果尝试几次没找到新对话按钮，可能已经是空白新页面了，直接进入下一步
                    if (attempts > 3) {{
                        step = 1; 
                        attempts = 0; // 重置计数器给下一步用
                    }}
                }}

                // 第 1 步：点击“专家模式” 或 “深度思考” 按钮
                if (step === 1) {{
                    // 兼容 DeepSeek 界面上可能叫 "专家模式" 或 "深度思考"
                    let expertBtn = Array.from(document.querySelectorAll('div, button, span')).find(el => 
                        el.textContent && (el.textContent.trim() === '专家模式' || el.textContent.trim() === '深度思考')
                    );

                    if (expertBtn) {{
                        // 检查一下是否已经被选中了(避免重复点击取消了)
                        let isSelected = expertBtn.closest('div').className.includes('active') || expertBtn.closest('div').style.color.includes('blue');
                        if (!isSelected) {{
                            expertBtn.click();
                        }}
                        step = 2; // 无论点没点，进入下一步输入阶段
                        return;
                    }}

                    if (attempts > 5) {{
                        // 找了几秒都没找到专家模式按钮（可能被缓存隐藏了或改版了），强行进入下一步防止卡死
                        step = 2;
                    }}
                }}
                ...
                if (step === 2) {{
                    ...
                    if (textarea) {{
                        clearInterval(fillInterval); // 所有前置操作完成，停止轮询
                        textarea.focus();
                        ...
                        // 延迟 800ms 等待前端状态更新后自动触发发送
                        setTimeout(function() {{
                            let sendBtn = document.querySelector('div[role="button"][aria-label*="发送"]') ||
                                          document.querySelector('button[aria-label*="发送"]') ||
                                          document.querySelector('.ds-send-button') ||
                                          document.querySelector('div[aria-disabled="false"]');
                ...
                if (attempts >= 25) {{
                    clearInterval(fillInterval); // 超时保险
                }}
            }}, 600); // 加快轮询频率到 0.6 秒
        }})();
        """
...
class SessionInterface(QWidget):
        ...
        if dlg.exec():
            ...
            try:
                ...
                with conn.cursor() as c:
                    ...
                    """, (data["server_ip"], data["name"], "system", "2742848", data["linux_user"], data["linux_pwd"],
                          data["port"]))
                conn.commit()
                conn.close()
                self.load_databases()
        ...
        if dlg.exec():
            ...
            try:
                ...
                with conn.cursor() as c:
                    c.execute("""
                        UPDATE db_server 
                        SET server_ip=%s, name=%s, linux_user=%s, linux_pwd=%s, port=%s
                        WHERE id=%s
    ...
    def load_sessions(self):
        ...
        query = f"""
            SELECT rownum rn, 
                   to_char(logon_time, 'YYYY-MM-DD HH24:MI:SS') start_time,
        ...
        db = ConnOracle(
            username="system",
            password="2742848",
            host=self.current_db_conn_info['server_ip'],
            port=1521,
            service_name="orcl"
        )
        db.connectorcl()
        ...
        try:
            ...
            for i, r in enumerate(records):
                ...
                items = [str(r[0]), str(r[1]), str(r[2]), str(r[3]), str(r[4]), str(r[5]),
                         str(r[6]), str(r[7]), str(r[8]), preview_sql, cache_key]
    ...
    def fetch_sql_text(self, selected_rows):
        ...
        cache_key = self.ui.session_table.item(first_row, 10).text()
        ...
        sql_id = sess_info['sql_id']
        child_num = sess_info['child_number']
        ...
        try:
            sql_res = db.fetch_all(
                f"SELECT sql_fulltext FROM v$sql WHERE sql_id='{sql_id}' AND child_number={child_num}")
            ...
            if self.ui.chk_bind.isChecked():
                bind_query = f"SELECT name, value_string FROM v$sql_bind_capture WHERE sql_id='{sql_id}' AND child_number={child_num}"
                binds = db.fetch_all(bind_query)
                if binds:
                    for b_name, b_val in binds:
    ...
    def fetch_execution_plan_internal(self, selected_rows):
        ...
        try:
            ...
            self.ui.plan_text.setPlainText(f"-- 执行计划提取异常: {str(e)} --")
        finally:
            db.disconnect()

    # 👈 核心改进：呼出 deepseek 智能分析弹窗
    def run_ai_analysis(self):
        ...
        # ⭐ 核心修复：如果由于“懒加载”机制导致 SQL 文本框为空，我们去后台动态反查
            ...
            if selected_rows:
                ...
                if sess_info and sess_info['sql_id']:
                    ...
                    try:
                        ...
                        sql_text = f"-- 后台动态获取 SQL 异常: {str(e)} --"
                    finally:
                        db.disconnect()

        # 若经过反查依然没有 SQL，给个缺省提示防报错
        if not sql_text:
            sql_text = "-- 未能捕获到源 SQL 语句 --"

        InfoBar.success(
            "AI 引擎已启动",
            "正在为您打开 DeepSeek 分析器...\n(首次使用请在弹窗内登录账号，后续会自动保持登录)",
            duration=4000,
            parent=self.window()
        )
    ...
    def preview_kill_session(self, selected_rows):
        ...
        for idx, model_index in enumerate(selected_rows):
            cache_key = self.ui.session_table.item(model_index.row(), 10).text()
    ...
    def fetch_statement_objects(self, selected_rows):
        ...
        for index in selected_rows:
            cache_key = self.ui.session_table.item(index.row(), 10).text()
            sess_info = self.current_sessions_cache.get(cache_key)
            if sess_info and sess_info['sql_id']:
