...
class Ui_Inca(object):
    def setupUi(self, IncaInterface):
        IncaInterface.setObjectName("IncaInterface")
        self.mainLayout = QVBoxLayout(IncaInterface)
        ...
        self.btn_refresh = PushButton("刷新", IncaInterface)
        self.btn_refresh.setIcon(FluentIcon.SYNC)

        self.btn_copy = PushButton("复制", IncaInterface)
        self.btn_copy.setIcon(FluentIcon.COPY)

        self.btn_add = PushButton("新增", IncaInterface)
        self.btn_add.setIcon(FluentIcon.ADD)

        self.btn_edit = PushButton("修改", IncaInterface)
        self.btn_edit.setIcon(FluentIcon.EDIT)

        self.btn_delete = PushButton("删除", IncaInterface)
        self.btn_delete.setIcon(FluentIcon.DELETE)

        self.btn_save = PushButton("保存", IncaInterface)
        self.btn_save.setIcon(FluentIcon.SAVE)

        self.btn_select_all = PushButton("全选", IncaInterface)
        self.btn_select_all.setIcon(FluentIcon.ACCEPT)

        self.btn_login = PushButton("登录", IncaInterface)
        self.btn_login.setIcon(FluentIcon.PEOPLE)

        self.btn_batch_edit = PushButton("批量修改", IncaInterface)
        self.btn_batch_edit.setIcon(FluentIcon.UPDATE)

        for btn in [self.btn_refresh, self.btn_copy, self.btn_add, self.btn_edit,
                    self.btn_delete, self.btn_save, self.btn_select_all, self.btn_login, self.btn_batch_edit]:
        ...
        # self.btn_query = PushButton("查询", IncaInterface)
        # self.btn_query.setIcon(FluentIcon.SEARCH)

        self.search_input = SearchLineEdit(IncaInterface)
        self.search_input.setPlaceholderText("输入关键字...")
        self.search_input.setFixedWidth(200)

        self.type_label = StrongBodyLabel("服务器类型:", IncaInterface)
        self.radio_erpnew = RadioButton("erpnew", IncaInterface)
        self.radio_erpold = RadioButton("erpold", IncaInterface)
        self.radio_wms = RadioButton("wms", IncaInterface)
        self.radio_erpnew.setChecked(True)

        self.path_label = StrongBodyLabel("程序路径:", IncaInterface)
        self.path_input = LineEdit(IncaInterface)
        self.path_input.setPlaceholderText("请选择或输入路径...")
        self.path_input.setMinimumWidth(250)
        self.btn_browse = PushButton("选择文件", IncaInterface)

        #self.filterLayout.addWidget(self.btn_query)
        self.filterLayout.addWidget(self.search_input)

        self.filterLayout.addSpacing(24)
        self.filterLayout.addWidget(self.type_label)
        self.filterLayout.addWidget(self.radio_erpnew)
        self.filterLayout.addWidget(self.radio_erpold)
        self.filterLayout.addWidget(self.radio_wms)
        ...
        # ================= 3. 主体：数据表格 =================
        self.tableWidget = TableWidget(IncaInterface)
        self.tableWidget.setWordWrap(False)

        headers = ['服务器ID', '服务器IP', '服务器名称', '服务器用户名', '服务器密码', '服务器类型',
                   '服务器所有者ID', '服务器所有者', '状态']
